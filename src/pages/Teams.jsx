...
export default function Teams() {
  ...
  const { allTeams, totalChampionshipPoints } = useMemo(() => {
    if (!driversData || sessionKey === "invalid")
      return { allTeams: [], totalChampionshipPoints: 0 };
  ...
  const dominancePercentage = topTeam
    ? Math.min(100, Math.round((topTeam.points / maxScorablePoints) * 100))
    : 0;
      ...
      <div style={{ padding: "0 5vw" }}>
        {isLoading ? (
          <div style={{ padding: "5rem 0" }}>
            <LoadingState
              message={`Syncing ${selectedYear} telemetry & constructor data...`}
            />
          </div>
        ) : errorDrivers || errorChamp ? (
          <div style={{ padding: "5rem 0" }}>
            <ErrorState
              message={errorDrivers || errorChamp}
              onRetry={() => {
                refetchDrivers();
                refetchChamp();
              }}
          ...
          <div className="state-container" style={{ padding: "5rem 0" }}>
            ...
            >
              ERR: NO_DATA_FOUND_FOR_SEASON_{selectedYear}
            </p>
          </div>
        ) : (
