...
export default function Events() {
  ...
  const getActualRound = (meeting) => {
    const index = meetings.findIndex(
      (m) => m.meeting_key === meeting.meeting_key,
    );
    if (index === -1) return 1;
  ...
  useEffect(() => {
    ...
    const fetchCalendar = async () => {
      ...
      try {
        ...
        const fetchRaces = f1Api
          .getSessions(selectedYear, "Race")
          .catch((err) => {
            console.warn("Failed to fetch races:", err);
            return [];
          });
        const fetchSprints = f1Api
          .getSessions(selectedYear, "Sprint")
          .catch((err) => {
        ...
        let validMeetings = meetingsData.filter(
          (m) => m.meeting_name !== "Pre-Season Testing",
        );

        if (validMeetings.length === 0) {
          console.log(
            `No events found for ${selectedYear}, retrying in 10s...`,
          );
          if (isMounted) {
  ...
  const sprintMeetings = useMemo(() => {
    return meetings.filter((m) =>
      sprintSessions.some((s) => s.meeting_key === m.meeting_key),
    );
      ...
      try {
        ...
        if (isMounted) {
          setLeaderboardData({
            ...
            loading: false,
            error:
              "Failed to connect to F1 telemetry server. Please try again.",
          });
        }
      }
    };
      ...
      >
        <div
          style={{
            ...
            inset: 0,
            background:
              "radial-gradient(circle at right top, rgba(225,6,0,0.12), transparent 50%)",
            zIndex: 0,
          }}
        />
      ...
      <div style={{ padding: "0 5vw" }}>
        {loading ? (
          <div style={{ padding: "10vh 0" }}>
            <LoadingState
              message={`Plotting the ${selectedYear} events calendar...`}
            />
          </div>
          ...
          <div style={{ padding: "10vh 0" }}>
            <ErrorState
              message={error}
              onRetry={() => setSelectedYear(selectedYear)}
            />
          </div>
        ) : meetings.length === 0 ? (
          <div className="state-container" style={{ padding: "10vh 0" }}>
            ...
            >
              EVENTS NOT YET SCHEDULED FOR {selectedYear}
            </p>
          </div>
        ) : (
              ...
              {nextEventData && (
                <section style={{ marginBottom: "2rem" }}>
                  ...
                  >
                    ...
                    <span
                      style={{
                        fontSize: "0.7rem",
                        background:
                          nextEventData.type === "Sprint Race"
                  ...
                  <MeetingCard
                    ...
                    showCountdown={true}
                    onClick={
                      nextEventData.meeting.is_cancelled
                        ? null
                        : () =>
                            setSelectedEvent({
                              meeting: nextEventData.meeting,
              ...
              <section>
                ...
                {sprintMeetings.length === 0 ? (
                  <div style={{ padding: "2rem 0" }}>
                    ...
                    >
                      No sprint events scheduled for the {selectedYear} season.
                    </p>
                  </div>
                ) : (
                  <div
                  ...
                  >
                    {sprintMeetings.map((meeting, index) => {
                      const isNext =
                        nextEventData &&
                        nextEventData.type === "Sprint Race" &&
                        nextEventData.meeting.meeting_key ===
                          meeting.meeting_key;
                      const session = sprintSessions.find(
                        (s) => s.meeting_key === meeting.meeting_key,
                      );
              ...
              <section>
                ...
                >
                  {meetings.map((meeting, index) => {
                    const isNext =
                      nextEventData &&
                      nextEventData.type === "Grand Prix" &&
                      nextEventData.meeting.meeting_key === meeting.meeting_key;
                    const session = raceSessions.find(
                      (s) => s.meeting_key === meeting.meeting_key,
                    );
                    const isCancelled = meeting.is_cancelled === true;
            ...
            >
              ...
              >
                ...
                <div>
                  ...
                  >
                    {selectedEvent.meeting.circuit_short_name} •{" "}
                    {selectedEvent.meeting.location},{" "}
                    {selectedEvent.meeting.country_name}
                  </div>
                </div>
              </div>
                  ...
                  <div>
                    ...
                    <div style={{ overflowX: "auto" }}>
                      ...
                      >
                        ...
                        <tbody>
                          {(selectedEvent.type === "Sprint"
                            ? leaderboardData.sprintResults
                            : activeTab === "race"
                              ? leaderboardData.raceResults
                              : leaderboardData.qualifyingResults
                          ).map((row, idx) => {
                              ...
                              >
                                ...
                                <td style={{ padding: "1rem", color: "#fff" }}>
                                  ...
                                  >
                                    <span
                                      onClick={() => {
                                        const slug = getDriverSlug(
                                          driver.givenName,
                                          driver.familyName,
                                        );
                                        const driverState = {
                                          driver_number: parseInt(
                                  ...
                                  <>
                                    ...
                                    >
                                      {(() => {
                                        ...
                                        const effectiveGridPos =
                                          gridPos === 0 ? 20 : gridPos;
                                        const diff =
                                          effectiveGridPos - finishPos;
